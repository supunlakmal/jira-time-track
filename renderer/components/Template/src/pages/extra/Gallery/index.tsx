              ...
              <div className='w-full filters-group-wrap mb-3'>
                <div className='flex justify-center mb-5'>
                  <ul className='filter-options flex flex-wrap gap-4 justify-center'>
                    <li className={`${(category) === 'all' ? 'active' : ''}`}><Link href='#' className='btn' onClick={() => filterImages('all')}>All Items</Link></li>
                    <li className={`${(category) === 'design' ? 'active' : ''}`}><Link href='#' className='btn' onClick={() => filterImages('design')}>Design</Link></li>
                    <li className={`${(category) === 'creative' ? 'active' : ''}`}><Link href='#' className='btn' onClick={() => filterImages('creative')}>Creative</Link></li>
                    <li className={`${(category) === 'digital' ? 'active' : ''}`}><Link href='#' className='btn' onClick={() => filterImages('digital')}>Digital</Link></li>
                    <li className={`${(category) === 'photography' ? 'active' : ''}`}><Link href='#' className='btn' onClick={() => filterImages('photography')}>Photography</Link></li>
                  </ul>
                </div>
              </div>
                  ...
                  return (
                    <a key={idx} className='xl:w-1/4 lg:w-1/3 md:w-1/2 p-3 inline-block picture-item image-popup' href={slide.image.src}>
                      <div className='relative block overflow-hidden rounded group transition-all duration-500'>
                        <Image src={slide.image.src} className='rounded transition-all duration-500 group-hover:scale-105' alt='work-image' />
        ...
        <div className='card'>
          ...
          <div className='p-6'>
            ...
            <div className='grid xl:grid-cols-5 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-5'>
              {(images || []).map((item, idx) => {
                return (
                  <Image key={idx} alt='gallery' className='object-cover object-center rounded' src={item.src} />
                )
              })}
            </div>
          </div>
              ...
              {(mixImages || []).map((item, idx) => {
                return (
                  <div key={idx} className={`${(mixImages[idx].isBIG) ? 'lg:col-span-2 lg:row-span-2' : ''}`}>
                    <Image alt='gallery' className='block object-cover object-center w-full h-full rounded' src={item.src} />
                  </div>
                )
              })
              }
              ...
              {(captionsImages || []).map((item, idx) => {
                return (
                  <div key={idx} className={`${(captionsImages[idx].isBIG) ? 'py-32 px-10 lg:col-span-2' : 'sm:py-24 py-16 sm:px-10 px-6'} flex flex-wrap w-full bg-gray-100 relative`}>
                    <Image alt='gallery' className='w-full object-cover h-full object-center block opacity-25 absolute inset-0 rounded' src={item.src} />
              ...
              {(hoverEffectImages || []).map((item, idx) => {
                return (
                  <div key={idx} className='flex relative'>
                    <Image alt='gallery' className='absolute w-full h-full object-cover object-center rounded' src={item.src} />
          ...
          <div className='p-6'>
            ...
            <div className='grid lg:grid-cols-5 md:grid-cols-3 sm:grid-cols-1 gap-5'>
              {(galleryfilter || []).map((item, idx) => {
                return (
                  <Image key={idx} alt='gallery' className={`${item.filter} object-cover object-center rounded filter`} src={item.src} />
