    ...
    <>
      <Dropzone onDrop={(acceptedFiles) => handleAcceptedFiles(acceptedFiles, onFileUpload)}>
        {({ getRootProps, getInputProps }) => (
          <div className="dropzone mt-2 flex justify-center items-center">
            <div className="dz-message needsclick flex items-center flex-col" {...getRootProps()}>
              <input {...getInputProps()} />
      ...
      {showPreview && selectedFiles.length > 0 && (
        <div className="dropzone-previews mt-3">
          {(selectedFiles || []).map((file, idx) => {
            return (
              <div className="mt-1 mb-0 shadow-none border rounded" key={idx + '-file'}>
                <div className="p-2">
                  <div className="flex items-center justify-between gap-2 w-full">
                    <div className='flex items-center gap-2 overflow-hidden'>
                      ...
                      <div className="ps-0 overflow-hidden">
                        <Link href="#" className="text-muted fw-bold truncate">
                          {file.name}
                        </Link>
                        <p className="mb-0">
                    ...
                    <div className="text-end">
                      <Link href="#" className="p-3 text-muted shadow-none">
                        <i className="mgc_close_line text-base" onClick={() => removeFile(file)} />
                      </Link>
                    </div>
                  </div>
                </div>
